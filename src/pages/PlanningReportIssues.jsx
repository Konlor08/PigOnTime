// src/pages/PlanningReportIssues.jsx
...
async function compressImage(
  file,
  { maxEdge = 1600, targetBytes = 1_000_000, q0 = 0.85 } = {}
) {
  ...
  if (blob.size > targetBytes)
    throw new Error(`ไฟล์รูปยังเกิน 1MB (${Math.round(blob.size / 1024)} KB)`);
  return new File([blob], `${(file.name || "image").split(".")[0]}.jpg`, {
    type: "image/jpeg",
  });
}
...
function SearchBox({ list, value, onChange }) {
  ...
  const results = useMemo(() => {
    ...
    return list.filter((x) =>
      [x.date, x.factory, farmLabel(x.farm), x.plan_id, x.site]
        .join(" ")
        .toLowerCase()
        .includes(s)
    );
  ...
  return (
    <div className="rounded-xl border bg-white/90 p-3">
      <input
        ...
        placeholder="ค้นหา (วันที่/ฟาร์ม/โรงงาน/รหัสแผน/SITE)"
        ...
        {results.map((g) => (
          ...
          >
            ...
            <div className="text-xs text-gray-600">
              SITE: {g.site || "-"} · โรงงาน: {g.factory || "-"} · แผน #{g.plan_id}
...
/* ---------- Page (Planning) ---------- */
export default function PlanningReportIssues() {
  // ผู้ใช้ (เก็บใน localStorage ตามระบบเดิม)
  let me = null;
  try {
  ...
  /* โหลดคิว: ย้อนหลัง 7 → ล่วงหน้า 7 วัน (ฝ่าย Planning ดูได้ทุกแผน) */
    ...
    try {
      ...
      const items =
        (plans || []).map((p) => ({
          ...
          farm: {
            id: null, // ไม่มี join farms สำหรับ Planning
            plant: p.plant,
            branch: p.branch,
            house: p.house,
      ...
      // 2) เช็คว่ามี issue แล้วหรือยัง (ใช้ตารางเดิม plan_issues)
      const { data: existed } = await supabase
        .from("plan_issues") // << เปลี่ยนชื่อ table ได้หากคุณมีตารางเฉพาะของ Planning
        .select("plan_id")
        .in(
          "plan_id",
          items.map((x) => x.plan_id)
        );
      ...
      setQueue(
        items
          ...
          .sort(
            (a, b) =>
              a.date.localeCompare(b.date) || a.plan_id - b.plan_id
          )
      );
    } catch (e) {
  ...
  useEffect(() => {
    const go = async () => {
      ...
      const { data: iss } = await supabase
        .from("plan_issues") // << เปลี่ยนชื่อ table ได้หากมีของ Planning
      ...
      if (iss) {
        ...
        const { data: fs } = await supabase
          .from("plan_issue_files") // << เปลี่ยนชื่อ table ได้หากมีของ Planning
  ...
  const onPick = async (e) => {
    ...
    try {
      ...
      for (const f of chosen.slice(0, remain)) {
        ...
        out.push({
          file: ff,
          previewUrl: isImage(ff.type) ? URL.createObjectURL(ff) : null,
          mime: ff.type,
        });
      }
  ...
  const removeAttached = async (id) => {
    ...
    await supabase.from("plan_issue_files").delete().eq("id", id); // << เปลี่ยนได้หากมีตาราง Planning
  };
    ...
    try {
      ...
      if (!iid) {
        const { data: ins, error: ei } = await supabase
          .from("plan_issues") // << เปลี่ยนได้หากมีตาราง Planning
          .insert({
            plan_id: sel.plan_id,
            delivery_date: sel.date,
            farm_id: null, // ฝ่าย Planning ไม่ผูก farms.id
            ah_id: me?.id || null, // เก็บผู้ใช้ที่บันทึก (อาจใช้เป็น planner_id)
            title: title.trim(),
            detail: detail || null,
            severity,
            status,
      ...
      for (const it of files) {
        ...
        const { error: eUp } = await supabase.storage
          .from("issue-files") // << ถ้ามี bucket สำหรับ Planning แยก เปลี่ยนตรงนี้
          .upload(path, f, {
            contentType: f.type,
            upsert: false,
            cacheControl: "3600",
        ...
        const { data: pub } = await supabase.storage
          .from("issue-files") // << เปลี่ยนให้ตรงกับบัคเก็ตข้างบน
          .getPublicUrl(path);
        ...
        const { error: eRec } = await supabase.from("plan_issue_files").insert({
          ...
          mime_type: f.type,
          file_bytes: f.size,
        }); // << เปลี่ยน table ได้หากมีของ Planning
        if (eRec) throw eRec;
      }
      ...
      setQueue((old) =>
        old.map((x) =>
          x.plan_id === sel.plan_id ? { ...x, has_issue: true } : x
        )
      );
      ...
      <header className="bg-emerald-600 text-white">
        <div className="mx-auto max-w-6xl px-4 py-3 flex items-center justify-between">
          <h1 className="text-2xl font-semibold">
            แจ้งปัญหา (ฝ่าย Planning) — ย้อนหลัง 7 วัน ↔ ล่วงหน้า 7 วัน
          </h1>
          <Link to="/planning" className="rounded-md bg-white/10 px-4 py-2 hover:bg-white/20">
            กลับหน้า Planning
          </Link>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-6xl px-4 py-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-2">
            ...
            {!busy && !queue.length && (
              <div className="text-gray-500 mt-3">ไม่มีคิวในช่วงเงื่อนไข</div>
            )}
          </div>
          ...
          <div className="space-y-3">
            <div className="rounded-xl border bg-white/90 p-3">
              ...
              <div className="text-sm text-gray-700 mb-2">
                ...
                <br />
                SITE: <b>{sel?.site || "-"}</b> · โรงงาน: <b>{sel?.factory || "-"}</b> · แผน #{sel?.plan_id ?? "-"}
              </div>
              ...
              <input
                ...
                placeholder="เช่น ข้อมูลแผนไม่ครบ, ต้องเลื่อนเวลา, ฯลฯ"
              ...
              <div className="text-xs text-gray-600 mb-2">
                ไฟล์รูปจะย่ออัตโนมัติไม่เกิน 1MB
              </div>
                ...
                <div className="mb-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {attached.map((f) => (
                      <div
                        key={f.id}
                        className="flex items-center gap-2 text-xs border rounded px-2 py-1 bg-gray-50"
                      >
                        <a
                          href={f.file_url}
                          target="_blank"
                        ...
                        <button
                          ...
                          onClick={onAnyAction(() => removeAttached(f.id))}
                        >
                          ลบ
                        </button>
                      </div>
