// src/pages/TransportTracking.jsx
import React, {
  useCallback,
  useEffect,
  useMemo,
...
export default function TransportTracking() {
  ...
  const [issueCountByPlan, setIssueCountByPlan] =
    useState({}); // plan_id -> count issues

  const [, setFactories] = useState([]); // เผื่อใช้
  const [factoryBySite, setFactoryBySite] = useState({}); // site -> {name,lat,lng}

  const [hasSitePrivilege, setHasSitePrivilege] =
    useState(true); // false = ไม่มี site ผูก
  ...
  /* ---------- โหลดแผน + session + พิกัด + issues ---------- */
  const loadQueues = useCallback(async () => {
    setErr("");
    setBusy(true);
    try {
      if (!meId)
        throw new Error("ไม่พบผู้ใช้ปัจจุบัน");

      // 1) SITE ที่ผู้ใช้นี้รับผิดชอบ (ใช้ planning_site_relations)
      let mySites = [];
      try {
        const { data: rels, error: eRel } =
          await supabase
        ...
        console.log(
          "loadQueues site relation error:",
          e?.message
        );
        mySites = [];
      }
      ...
      if (!mySites.length) {
        setHasSitePrivilege(false);
      } else {
        setHasSitePrivilege(true);
      }

      // 2) แผนในช่วงเวลา (กรองตาม SITE ถ้ามี)
      let q = supabase
      ...
      // สมมติ factory = site
      if (mySites.length)
        q = q.in("factory", mySites);

      const { data: plans, error: ePlans } =
        await q;
      ...
      // 5) จำนวน issues ต่อแผน
      const issueCount = {};
      if (planIds.length) {
        const { data: iss, error: eIssues } =
          await supabase
            .from("plan_issues")
            .select("id, plan_id")
            .in("plan_id", planIds);
        if (eIssues) {
          ...
          (iss || []).forEach((x) => {
            issueCount[x.plan_id] =
              (issueCount[x.plan_id] || 0) + 1;
          });
        }
      }
      ...
      toastOk("โหลดข้อมูลแผนและสถานะแล้ว");
    } catch (e) {
      toastErr(
      ...
      <header className="bg-amber-600 text-white">
        <div className="mx-auto max-w-6xl px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {/* ปุ่มกลับหน้า home ตาม role */}
            {["planning", "animalhusbandry", "manager"].includes(
              role
            ) && (
              <button
                type="button"
      ...
      <main className="mx-auto max-w-6xl px-4 py-6 space-y-4">
        ...
        {!hasSitePrivilege && (
          <div className="rounded-lg border border-amber-200 bg-amber-50 text-amber-800 px-3 py-2">
            คุณยังไม่ได้รับสิทธิผูกกับ site ใด ๆ
        ...
        <div className="rounded-xl border border-amber-200 bg-white p-2">
          <div className="px-2 py-1 text-sm text-gray-600">
            แผนที่แสดงตำแหน่ง ฟาร์ม / โรงงาน /
            รถขนส่ง (เฉพาะคิวที่ยังขนส่ง)
          </div>
            ...
            >
              ...
              {overviewMarkers.map(
                ({ plan, pos }) => {
                  ...
                  const facName =
                    factoryBySite[plan.factory]
                      ?.name ||
                    plan.factory ||
                    "-";
                  const issues =
                    issueCountByPlan[plan.id] ||
                    0;
                  const etaMin =
                    calcEtaMin(plan);
        ...
        <div className="space-y-3">
          ...
          {!busy && filtered.length === 0 && (
            <div className="rounded-lg border border-amber-200 bg-white p-4 text-gray-600">
              ตอนนี้ยังไม่มีคิวที่อยู่ระหว่างขนส่งสำหรับ
              site ที่คุณรับผิดชอบ
            </div>
          )}
