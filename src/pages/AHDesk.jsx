...
/* Fix Leaflet marker assets (Vite) + icon — ประกาศครั้งเดียว */
...
export default function AHDesk() {
  ...
  const [rows, setRows] = useState([]);           // แผน (ของฟาร์มที่ฉันดูแล)
  ...
  // โรงงาน (ไว้หาพิกัดเพื่อคำนวณ ETA)
  ...
  /* โหลดแผน/สถานะ/พิกัดล่าสุด ของฟาร์มที่ฉันดูแล (-7..+7 วัน) */
  const loadQueues = useCallback(async () => {
    setErr(""); setBusy(true);
    try {
      if (!me?.id) throw new Error("ไม่พบผู้ใช้ปัจจุบัน");

      // 1) ฟาร์มที่ AH รับผิดชอบ
      const { data: rel, error: e1 } = await supabase
        ...
        .eq("status", "active");
      if (e1) throw e1;
      const myFarms = (rel || []).map((r) => r.farms).filter(Boolean);          // [{id,plant,branch,house,farm_name}]
      const farmIds = new Set((rel || []).map(r => r.farm_id));

      // 2) แผนช่วง -7..+7 (แล้วค่อย filter ด้วย farm_id หรือ keys)
      const { data: plans, error: e2 } = await supabase
        .from("planning_plan_full")
        .select("id, delivery_date, delivery_time, plant, branch, house, farm_id, farm_name, plate, factory, quantity")
        .gte("delivery_date", start)
        .lte("delivery_date", end)
        .order("delivery_date", { ascending: true });
      if (e2) throw e2;

      // จับคู่เฉพาะแผนของฟาร์มที่ฉันดูแล (รองรับกรณี farm_id เป็น null)
      const isMyFarm = (p) => {
        if (p.farm_id && farmIds.has(p.farm_id)) return true;
        return myFarms.some(f =>
          (f.plant || "") === (p.plant || "") &&
          (f.branch || "") === (p.branch || "") &&
          (f.house || "") === (p.house || "") &&
  ...
  /* ETA: จากพิกัดล่าสุด → โรงงานปลายทาง */
  ...
  const overviewCoords = useMemo(() => {
    ...
    // รวมจุดโรงงานปลายทางด้วย (ถ้ามีพิกัด)
    filtered.forEach(p => {
  ...
  /* logout */
  const doLogout = () => { try { localStorage.removeItem("user"); } catch {/* intentionally empty */} navigate("/login", { replace: true }); };
    ...
    <div className="min-h-screen bg-emerald-50">
      ...
      <header className="bg-emerald-700 text-white">
        <div className="mx-auto max-w-6xl px-4 py-3 flex items-center justify-between">
          <h1 className="text-2xl font-semibold">AH Desk</h1>
          <div className="flex items-center gap-2">
            <Link to="/ah/report-lite" className="rounded-md bg-white/10 px-3 py-2 hover:bg-white/20">แจ้ง/แก้ไขรายงาน</Link>
            <button
              type="button"
              onClick={doLogout}
              className="rounded-md bg-emerald-200 text-emerald-900 px-4 py-2 font-semibold hover:bg-emerald-300 active:bg-emerald-400"
            >
              Logout
            </button>
          </div>
      ...
      <main className="mx-auto max-w-6xl px-4 py-6 space-y-4">
        ...
        {/* ✅ แผนที่เดียว แสดงรถทุกคันของฟาร์มที่ดูแล + จุดโรงงาน */}
        <div className="rounded-xl border border-emerald-200 bg-white p-2">
          <div className="px-2 py-1 text-sm text-gray-600">แผนที่รวมรถขนส่งจากฟาร์มที่รับผิดชอบ</div>
          <div className="h-[520px] rounded-lg overflow-hidden border border-emerald-200">
            <MapContainer className="h-full w-full" center={[13.736717, 100.523186]} zoom={12} scrollWheelZoom>
              ...
              {/* จุดโรงงานปลายทาง (เฉพาะที่มีพิกัด) */}
              {filtered.map((p) => {
                const fac = factoryBySite[p.factory];
              ...
              {overviewMarkers.map(({ plan, pos }) => {
                ...
                return (
                  <Marker key={plan.id} position={[pos.lat, pos.lng]} icon={iconSmall}>
                    <Tooltip permanent direction="top" offset={[0, -10]}>
                      รถ {plan.plate || "-"} • {status} • ไป {facName}{etaMin ? ` • ETA ${etaMin}น.` : ""}
                    </Tooltip>
                    <Popup>
